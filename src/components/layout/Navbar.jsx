...
const FEATURED_CATEGORIES = [
  { label: 'Desktop', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>), href: '/products?category=desktop', sub: ['All-in-One PCs', 'Gaming Desktops', 'Mini PCs', 'Workstations', 'Desktop Accessories'] },
  { label: 'Drone', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>), href: '/products?category=drones', sub: ['DJI AIR', 'DJI Mavic', 'DJI Mini', 'Racing Drones', 'Drone Accessories'] },
  { label: 'Laptop', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 14l9-5-9-5-9 5 9 5z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"/></svg>), href: '/products?category=laptops', sub: ['Gaming Laptops', 'Ultrabooks', 'MacBooks', '2-in-1 Laptops', 'Business Laptops'] },
  { label: 'TV', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>), href: '/products?category=tvs', sub: ['Smart TVs', 'OLED TVs', 'QLED TVs', '4K TVs', '8K TVs'] },
  { label: 'Phone', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>), href: '/products?category=phones', sub: ['Smartphones', 'Tablets', 'Cases & Covers', 'Chargers', 'Screen Protectors'] },
  { label: 'Camera', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"/></svg>), href: '/products?category=cameras', sub: ['DSLR Cameras', 'Mirrorless', 'Action Cameras', 'Security Cameras', 'Lenses'] },
  { label: 'VR (Virtual Reality)', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/></svg>), href: '/products?category=vr', sub: ['VR Headsets', 'AR Glasses', 'VR Controllers', 'VR Games', 'VR Accessories'] },
  { label: 'Smart Watch', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>), href: '/products?category=smartwatch', sub: ['Apple Watch', 'Samsung Galaxy Watch', 'Fitness Trackers', 'Kids Watches', 'Watch Bands'] },
  { label: 'Gaming Console', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 011-1h3a1 1 0 001-1V4z"/></svg>), href: '/products?category=gaming', sub: ['PlayStation 5', 'Xbox Series X', 'Nintendo Switch', 'Gaming Controllers', 'Gaming Headsets'] },
  { label: 'Router', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.14 0M1.394 9.393c5.857-5.857 15.355-5.857 21.213 0"/></svg>), href: '/products?category=networking', sub: ['WiFi Routers', 'Mesh Systems', 'Modems', 'Network Switches', 'Range Extenders'] },
  { label: 'Entertainment', icon: (<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z"/></svg>), href: '/products?category=entertainment', sub: ['Streaming Devices', 'Projectors', 'Blu-ray Players', 'Soundbars', 'Media Players'] },
];
...
export default function Navbar() {
  ...
  const [wishlistPopKey, setWishlistPopKey] = useState(0);
  const prevCartItems = useRef(0);
  const prevWishlistCount = useRef(0);

  const { currency: selectedCurrency, currencies: CURRENCIES, setCurrency } = useCurrency();
  ...
  return (
    <>
      <style>{`
        ...
        .pc-builder-shimmer { position:absolute; top:0; left:-100%; width:60%; height:100%; background:linear-gradient(90deg,transparent,rgba(255,255,255,0.25),transparent); transform:skewX(-20deg); animation:pcShimmer 2.8s ease-in-out infinite; z-index:0; }
        .pc-builder-pulse { position:absolute; inset:0; border-radius:14px; box-shadow:0 0 0 0 rgba(232,81,122,0.6); animation:pcPulse 2.2s ease-out infinite; z-index:0; }
        @keyframes pcShimmer { 0%{left:-100%} 40%{left:140%} 100%{left:140%} }
        @keyframes pcPulse { 0%{box-shadow:0 0 0 0px rgba(232,81,122,0.55)} 60%{box-shadow:0 0 0 8px rgba(232,81,122,0)} 100%{box-shadow:0 0 0 0px rgba(232,81,122,0)} }
      `}</style>
        ...
        <div className="bg-gray-900 text-white text-xs overflow-hidden transition-all duration-150 ease-in-out"
          ...
          <div className="relative overflow-hidden h-9 flex items-center justify-center">
            {ANNOUNCEMENT_MESSAGES.map((msg, i) => (
              <span key={i} className="absolute inset-0 flex items-center justify-center transition-all duration-500 whitespace-nowrap text-xs font-medium tracking-wide"
                style={{ opacity: i === msgIndex ? 1 : 0, transform: i === msgIndex ? 'translateY(0)' : i < msgIndex ? 'translateY(-100%)' : 'translateY(100%)' }}>
                {msg}
              </span>
            ))}
          </div>
            ...
            <div className="container-custom flex items-center h-9 gap-4 text-sm text-gray-500">
              ...
              <div className="hidden md:flex items-center gap-3 flex-shrink-0">
                <div className="relative" ref={currencyRef}>
                  <button onClick={(e) => { e.stopPropagation(); setCurrencyOpen(!currencyOpen); setLangOpen(false); }}
                    className="flex items-center gap-1 hover:text-orange-500 transition-colors py-1 px-1.5 rounded hover:bg-orange-50">
                    <span>{selectedCurrency.flag}</span><span>{selectedCurrency.code}</span>
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/></svg>
                  </button>
                  {currencyOpen && (
                    <div className="absolute right-0 top-full mt-1 w-32 bg-white rounded-lg shadow-xl border border-gray-100 py-1 z-[100] animate-fade-in">
                      {CURRENCIES.map((c) => (
                ...
                <div className="relative" ref={langRef}>
                  <button onClick={(e) => { e.stopPropagation(); setLangOpen(!langOpen); setCurrencyOpen(false); }}
                    className="flex items-center gap-1 hover:text-orange-500 transition-colors py-1 px-1.5 rounded hover:bg-orange-50">
                  ...
                  {langOpen && (
                    <div className="absolute right-0 top-full mt-1 w-36 bg-white rounded-lg shadow-xl border border-gray-100 py-1 z-[100] animate-fade-in">
                      {LANGUAGES.map((l) => (
            ...
            <div className="flex items-center h-16 gap-4">
              ...
              <div className="hidden md:flex flex-1" style={{ marginLeft: '22px', marginRight: '360px' }}>
                <form className="flex w-full border-2 border-gray-200 rounded-xl overflow-hidden focus-within:border-orange-400 transition-all duration-200 focus-within:shadow-brand"
                  ...
                  <input type="text" placeholder="Search phones, laptops, TVs..."
                    value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                    className="flex-1 px-4 py-2.5 text-sm outline-none" />
                  <button type="submit"
                    className="shimmer-btn px-5 py-2.5 text-white text-sm font-medium transition-opacity hover:opacity-90 flex-shrink-0"
              ...
              <div className="flex items-center gap-3 ml-auto">
                ...
                <Link href="/pc-builder"
                  ...
                  <span className="pc-builder-shimmer" />
                  <span className="pc-builder-pulse" />
                  <svg className="w-4 h-4" style={{ position:'relative', zIndex:1, flexShrink:0 }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <span style={{ position:'relative', zIndex:1 }}>PC Builder</span>
                ...
                {user ? (
                  <div className="hidden md:block relative" ref={accountRef}>
                    <button onClick={() => setAccountOpen(!accountOpen)}
                      ...
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold transition-transform hover:scale-110"
                        style={{ background: 'linear-gradient(135deg,#e8517a,#f4874b)' }}>
                ...
                <button className="md:hidden p-2 rounded-lg text-gray-600 hover:bg-gray-100 transition-colors" onClick={() => setMobileOpen(!mobileOpen)}>
            ...
            <nav className="hidden md:flex items-center border-t border-gray-100">
              ...
              <div className="relative flex-shrink-0" ref={categoryRef}>
                <button onClick={() => { setCategoryOpen(!categoryOpen); setHoveredCat(null); }}
                  ...
                  Featured Category
                  <svg className="w-3 h-3 transition-transform duration-200" style={{ transform: categoryOpen ? 'rotate(180deg)' : 'rotate(0deg)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/>
                  </svg>
                </button>

                {categoryOpen && (
                  <div className="absolute top-full left-0 z-50 flex shadow-2xl border border-gray-100 rounded-b-xl overflow-hidden animate-fade-in" style={{ minWidth: '200px' }}>
                    <ul className="bg-white w-52 py-2 flex-shrink-0">
                      {FEATURED_CATEGORIES.map((cat, i) => (
                        <li key={cat.label} style={{ animationDelay: `${i * 30}ms` }}>
                          <Link href={cat.href}
                            ...
                            <span className="flex items-center gap-2">
                              <span className={`transition-colors ${hoveredCat === cat.label ? 'text-orange-500' : 'text-gray-400'}`}>{cat.icon}</span>
                              {cat.label}
                            </span>
                            {cat.sub && <svg className="w-3 h-3 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7"/></svg>}
                          </Link>
                        </li>
                      ))}
                    </ul>
                    {hoveredCat && (() => {
                      ...
                      return active?.sub ? (
                        <div className="bg-gray-50 border-l border-gray-100 w-48 py-3 flex-shrink-0 animate-fade-in">
                          <p className="px-4 pb-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">{active.label}</p>
                          <ul>
                            {active.sub.map((item) => (
                              <li key={item}>
                                <Link href={`${active.href}&sub=${item.toLowerCase().replace(/\s+/g, '-')}`}
                                  ...
                                  <span className="w-1 h-1 rounded-full bg-orange-400 flex-shrink-0" />{item}
                                </Link>
                              </li>
                            ))}
                          </ul>
              ...
              <div className="flex items-center ml-4" ref={navDropRef}>
                {NAV_LINKS.map((link) => (
                  <div key={link.label} className="relative"
                    ...
                    <Link href={link.href}
                      className="nav-link-animated flex items-center gap-1 px-4 py-3 text-base font-medium text-gray-600 hover:text-orange-500 transition-colors whitespace-nowrap">
                      {link.label}
                      {link.sub && <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/></svg>}
                    </Link>
                    {link.sub && hoveredNav === link.label && (
                      <div className="absolute top-full left-0 z-50 bg-white border border-gray-100 rounded-xl shadow-xl py-2 min-w-max animate-fade-in">
                        {link.sub.map((item) => (
                          <Link key={item} href={`${link.href}?tag=${item.toLowerCase().replace(/\s+/g, '-')}`}
              ...
              <div className="ml-auto flex items-center rounded-lg overflow-hidden border-2 border-gray-200 hover:border-orange-400 transition-all duration-200 flex-shrink-0 hover:shadow-brand">
                <button onClick={openCart}
                  ...
                  <div key={cartPopKey} className={cartPopKey > 0 ? 'icon-pop' : ''}>
                ...
                <button onClick={openCart}
                  className="shimmer-btn px-4 py-2 text-white text-sm font-bold flex-shrink-0 transition-opacity hover:opacity-90"
                  style={{ background: 'linear-gradient(135deg, #e8517a, #f4874b)' }}>
                  GO
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-40 bg-white pt-16 overflow-y-auto md:hidden animate-fade-in">
          <div className="p-4">
            <div className="flex gap-3 mb-4 pb-4 border-b border-gray-100">
              <select value={selectedCurrency.code} onChange={(e) => setCurrency(e.target.value)}
                className="flex-1 text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none">
                {CURRENCIES.map(c => <option key={c.code} value={c.code}>{c.flag} {c.code}</option>)}
              </select>
              <select value={selectedLang.code} onChange={(e) => setSelectedLang(LANGUAGES.find(l => l.code === e.target.value))}
                className="flex-1 text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none">
            ...
            {FEATURED_CATEGORIES.map((cat, i) => (
              <Link key={cat.label} href={cat.href} onClick={() => setMobileOpen(false)}
                className="flex items-center gap-3 py-3 text-gray-700 border-b border-gray-100 font-medium animate-slide-in-up"
                style={{ animationDelay: `${i * 40}ms` }}>
                <span className="text-gray-400">{cat.icon}</span>{cat.label}
              </Link>
            ))}
